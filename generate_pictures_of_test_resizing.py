...
def gen_images_v2(testname):
    dirname='pictures_test_resizing'
    mkdir(dirname)
    global colors
    with open(f'output_test_{testname}','rt') as f:
        data0 = eval(f.read())
    screen_dim  = get_screen_dim(data0[0])

    print(f'screen_dim={screen_dim}')
    i=Image.new("RGBA",(screen_dim), (255, 255, 255, 0))
    d=ImageDraw.Draw(i)
    ...
    for t in data0:
            ...
            for win in windows:
                ...
                if len(colors) < 1:
                    colors = cc()
                d.rectangle(win,fill="#"+colors.pop())
            fname=f"./{dirname}/img_{testname}_{count}.png"
            print(f"writing file: {fname} ")
            i.save(fname)
            count+=1

from os import mkdir
gen_images_v2('resizing')
